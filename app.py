...
def video_save(vpost):
    ...
    try:
        ...
        #vpost.url_vid = vpost.post_soup.find_all('video')[0].attrs['src']
        vpost.url_vid = parse_video_url(vpost)
        ...
        print("Downloading a video failed!")
        print(e)
...
def parse_video_url(post):
    oc = post.post_soup.find_all('a')[0].attrs['onclick']
    begin = oc.find('1080p')+8
    end = oc.find('"', begin + 1)
    return oc[ begin: end ].replace('\/', '/')
